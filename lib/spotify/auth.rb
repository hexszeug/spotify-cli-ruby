    ...
    module Auth
            ...
            class << self
                ...
                def set_token(token_hash)
                    unless token_hash.is_a? Hash
                        raise TokenError.new "token must be a hash"
                    end
                    ...
                    unless token.key?(:access_token) && token[:access_token].is_a?(String)
                        raise TokenError.new "missing access token"
                    end
                    ...
                    unless token.key?(:expires_at) && token[:expires_at].is_a?(Integer)
                        raise TokenError.new "missing expiration infomation"
                    end
                    unless token.key?(:refresh_token) &&
                                      token[:refresh_token].is_a?(String)
                        raise TokenError.new "missing refresh token" unless @token
                        @token.update token
                    else
                        @token = token
                    end
        ...
        module Login
            ...
            module CodeServer
                ...
                def CodeServer.start(state, &callback)
                    ...
                    begin
                        ...
                        raise OpenCodeServerError.new e
                    end
